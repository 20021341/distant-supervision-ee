...
def build_entities_system_prompt(include_hints: bool = True):
    entity_types_text = ""
...
def build_events_system_prompt(include_hints: bool = True):
    event_types_text = ""
...
def build_arguments_system_prompt():
...
def build_full_system_prompt(include_hints: bool = True):
    entity_types_text = ""
    for index, entity_type in enumerate(ENTITY_TYPES, start=1):
        entity_types_text += f"{index}. {entity_type}\n"
        if include_hints:
